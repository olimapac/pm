...
- `main.py` - app with `GET /api/hello`, Kanban routes (`GET /api/board`, `PATCH /api/columns/{id}`, `POST|PATCH|DELETE /api/cards...`, `POST /api/cards/move`), `POST /api/ai/test` (Zen `muse-spark-1.3-contributor-free` via `openai` client, key from `OPENCODE_API_KEY`), `POST /api/ai/chat` (board-aware: `call_zen_chat` + server-side `validate_ops`/`apply_patch`, returns `{reply, board, applied}`) and static files from `static/` served at `/`
